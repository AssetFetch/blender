...
class AF_PT_ProviderPanel(bpy.types.Panel):
	...
	def draw(self, context):
		...
		info_box.label(text="AssetFetch for Blender v0.1.0-alpha.",
		               icon="SETTINGS")
		info_box.label(text=f"Download directory: {af.download_directory}")
		info_box.label(text=f"Icon directory: {af.ui_image_directory}")
		info_box.label(
		    text=
		    "Unstable & lacking numerous features, use with caution & patience!"
		)

		# Add a text box to enter the URL
		layout.prop(context.window_manager.af,
		            "current_init_url",
		            text="Provider URL",
		            icon="URL")
		...
		# Headers
		if len(af.current_provider_initialization.provider_configuration.
		       headers.values()) > 0:

			for provider_header in af.current_provider_initialization.provider_configuration.headers.values(
			):
				layout.prop(provider_header,
				            "value",
				            text=provider_header.title)
		...
		layout.label(
		    text=af.current_connection_state.bl_rna.properties['state'].
		    enum_items[af.current_connection_state.state].description,
		...
		if af.current_provider_initialization.text.title != "":
			text_column.label(
			    text=af.current_provider_initialization.text.title,
			    icon="PLUGIN")
		else:
			text_column.label(text="(No title configured)", icon="PLUGIN")

		text_column.label(
		    text=af.current_provider_initialization.text.description)
		...
		if af.current_connection_state.user.is_set:
			...
			if (af.current_connection_state.user.display_name != ""):
				user_column.label(
				    text=af.current_connection_state.user.display_name,
				    icon="USER")
			if (af.current_connection_state.user.display_tier != ""):
				user_column.label(
				    text=af.current_connection_state.user.display_tier,
				    icon="WORKSPACE")
			if (af.current_connection_state.unlock_balance.is_set):
				user_column.label(
				    text=
				    f"{af.current_connection_state.unlock_balance.balance} {af.current_connection_state.unlock_balance.balance_unit}",
				    icon="RADIOBUT_ON")
