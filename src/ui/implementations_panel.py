...
class AF_PT_ImplementationsPanel(bpy.types.Panel):
	...
	def poll(self, context) -> bool:
		...
		return af.current_connection_state.state == "connected" and len(
		    af.current_asset_list.assets) > 0
	...
	def draw(self, context):
		...
		if len(af.current_implementation_list.implementations) > 0:
			...
			layout.template_list("UI_UL_list", "name",
			                     af.current_implementation_list,
			                     "implementations", af,
			                     "current_implementation_list_index")
			...
			if current_impl.is_valid:
				layout.label(
				    text="Implementation is readable. Ready to import.",
				...
				layout.label(text="Implementation is not readable.",
			...
			for step in current_impl.import_steps:
				...
				step_title = step.bl_rna.properties['action'].enum_items[
				    step.action].name
				step_action_icon = AF_ImportAction[step.action].icon_string()
				step_state_icon = AF_ImportActionState[
				    step.state].icon_string()
				...
				if step.action == AF_ImportAction.fetch_download.value:
					target_component = current_impl.get_component_by_id(
					    step.config['component_id'].value)
				...
				if step.completion > 0.0 and step.completion < 1.0 and step.state not in [
				    AF_ImportActionState.canceled.value,
				    AF_ImportActionState.failed.value
				]:
					row.progress(text=step_details,
					             factor=step.completion,
					             type="RING")
				else:
		...
		# We have already queried and found that there are no results...
			...
			no_results_box.label(
			    text="No implementations found for this query.",
			    icon="ORPHAN_DATA")
