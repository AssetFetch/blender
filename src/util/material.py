...
def get_or_create_material(material_name: str, af_namespace: str):
	...
	for existing_material in bpy.data.materials:
		if ("af_name" in existing_material) and ("af_namespace"
		                                         in existing_material):
			if existing_material[
			    'af_name'] == material_name and existing_material[
	...
	output.name = "OUTPUT"
	bsdf_shader = new_material.node_tree.nodes.new(
	...
	# Basic links
	new_material.node_tree.links.new(bsdf_shader.outputs['BSDF'],
...
def add_map_to_material(target_material: bpy.types.Material,
                        colorspace: af_constants.AF_Colorspace,
                        map: af_constants.AF_MaterialMap,
	...
	current_vertical_node_position = (
	    count_image_nodes(target_material.node_tree) - 1) * -300
	image_node.location.y = current_vertical_node_position

	# Connect
	target_material.node_tree.links.new(
	    target_material.node_tree.nodes['TEX_COORD'].outputs['UV'],
	...
	# Color Map
	if map in [
	    af_constants.AF_MaterialMap.albedo, af_constants.AF_MaterialMap.diffuse
	]:
		color_image_node = target_material.node_tree.links.new(
		    image_color_out, bsdf_inputs['Base Color'])

	# Normal Map
	if map in [
	    af_constants.AF_MaterialMap.normal_plus_y,
	    af_constants.AF_MaterialMap.normal_minus_y
	]:
		normal_map_node = target_material.node_tree.nodes.new(
		...
		target_material.node_tree.links.new(
		    ...
		    target_material.node_tree.nodes['BSDF'].inputs['Normal'])

	if map == af_constants.AF_MaterialMap.normal_plus_y:
		target_material.node_tree.links.new(image_node.outputs['Color'],
		                                    normal_map_node.inputs['Color'])

	if map == af_constants.AF_MaterialMap.normal_minus_y:
		# Green channel must be inverted
		# Separate Color
		separate_color_node = target_material.node_tree.nodes.new(
		...
		target_material.node_tree.links.new(
		    image_node.outputs['Color'], separate_color_node.inputs['Color'])
		# Invert Green
		invert_normal_y_node = target_material.node_tree.nodes.new(
		    type="ShaderNodeInvert")
		invert_normal_y_node.location.y = current_vertical_node_position
		invert_normal_y_node.location.x = 400
		target_material.node_tree.links.new(
		    separate_color_node.outputs['Green'],
		    invert_normal_y_node.inputs['Color'])
		# Combine again
		combine_color_node = target_material.node_tree.nodes.new(
		    type="ShaderNodeCombineColor")
		combine_color_node.location.y = current_vertical_node_position
		combine_color_node.location.x = 550
		target_material.node_tree.links.new(
		    invert_normal_y_node.outputs['Color'],
		    combine_color_node.inputs['Green'])
		target_material.node_tree.links.new(separate_color_node.outputs['Red'],
		                                    combine_color_node.inputs['Red'])
		target_material.node_tree.links.new(
		    separate_color_node.outputs['Blue'],
	...
	if map == af_constants.AF_MaterialMap.roughness:
		target_material.node_tree.links.new(image_color_out,
		                                    bsdf_inputs['Roughness'])

	# Glossiness
	if map == af_constants.AF_MaterialMap.glossiness:
		# Map needs to be inverted
		invert_roughness_node = target_material.node_tree.nodes.new(
		    type="ShaderNodeInvert")
		invert_roughness_node.location.y = current_vertical_node_position
		invert_roughness_node.location.x = 400
		target_material.node_tree.links.new(
		    image_color_out, invert_roughness_node.inputs['Color'])
		target_material.node_tree.links.new(
		    invert_roughness_node.outputs['Color'], bsdf_inputs['Roughness'])
	...
	if map == af_constants.AF_MaterialMap.height:
		displacement_node = target_material.node_tree.nodes.new(
		...
		target_material.node_tree.links.new(
		    ...
		    target_material.node_tree.nodes['OUTPUT'].inputs['Displacement'])
