	...
	def property_items():
		return [
		    ...
		    ("awaiting_input", "Awaiting Input",
		     "Configuration values are required in order to connect"),
		    ("connected", "Connected", "The connection has been established"),
		    ("connection_error", "Connection Error",
		     "An error occured while connecting to the provider")
		]
...
class AF_ImportAction(Enum):
	...
	def icon_string(self):
		...
		icons = {
		    ...
		    AF_ImportAction.import_loose_material_map_from_local_path:
		    "MATERIAL",
	...
	def property_items():
		return [
		    ...
		    ("fetch_from_zip_archive", "Extract File From ZIP Archive",
		     "Extract a file from a zip archive."),

		    # Import actions
		    ("import_obj_from_local_path", "Import OBJ",
		     "Imports obj file from local path."),  # component_id
		    ("import_usd_from_local_path", "Import USD",
		     "Imports USDA/C/Z file from a local path."),  # component_id
		    ("import_loose_material_map_from_local_path",
		     "Import loose material map",
		     "Adds a loose material map from a local path to a material."
		     ),  # component_id
		    ("import_loose_environment_from_local_path",
		     "Import a loose environment",
		     "Imports a loose HDR/EXR/... file and creates a world from it."
		     ),  # component_id

		    # Unlock actions
		    ("unlock", "Unlock Resource", ""),  # query_id
		    ("unlock_get_download_data", "Prepare Download",
		     "Prepare a download of a file after it has been unlocked."
		     ),  # component_id

		    # Misc actions
		    ("create_directory", "Create Directory", "Create a directory."
		     ),  # directory
		]
